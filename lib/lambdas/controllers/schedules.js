    ...
    switch (e.requestContext.httpMethod) {
      ...
      case "POST": {
        const createSchedule = new CreateScheduleCommand({
          ...
          Target: {
            Arn: process.env.SCHEDULER_ARN, // The target that is scheduled, usually a lambda function to trigger an EC2 creation.
            RoleArn: process.env.ROLE_ARN, // Role needed for scheduler to run the lambda function
          },
        });
